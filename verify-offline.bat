...
rem 독자 검증 로직 없음). legacy 변환/외부 저장소 의존 단계는 모두 제거되었다 -- JDK 1.8.0
rem family 게이트를 포함한 verify-standalone.bat 출력을 그대로 노출한다(우회/은폐 없음).
...
if "%STANDALONE_EXIT%"=="0" (
  ...
  echo [CORE_VERIFICATION_FAIL] verify-standalone.bat did not pass -- see its output above ^(this includes the JDK 1.8.0 family gate, which is never bypassed here^).
  exit /b 1
)
