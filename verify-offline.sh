...
# cmd.exe로 verify-standalone.bat을 위임 호출하는 thin delegator다(exact-JDK 게이트 재구현 없음).
...
if [ "$STANDALONE_EXIT" = "0" ]; then
  ...
  echo "[CORE_VERIFICATION_FAIL] verify-standalone.bat did not pass -- see its output above (this includes the exact-JDK gate, which is never bypassed or reimplemented here)."
  exit 1
fi
