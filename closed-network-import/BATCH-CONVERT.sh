#!/bin/sh
# 폐쇄망 batch 변환 entrypoint(Slice 99F). exact-JDK 게이트/compile/regression 로직을 여기서
# 재구현하지 않고 verify-standalone.bat에 전부 위임한다(이 프로젝트의 정규 entrypoint는 Windows
# batch다 -- 이 .sh는 cmd.exe를 통해 그 batch를 그대로 호출하는 best-effort 브리지일 뿐이다).
...
echo "== Closed-network batch conversion (delegates exact-JDK gate to verify-standalone.bat) =="
if ! command -v cmd.exe >/dev/null 2>&1; then
  echo "[BATCH_FAIL] cmd.exe not available in this shell -- verify-standalone.bat (the authoritative"
  echo "exact-JDK gate) cannot be invoked from here. This entrypoint does not duplicate its JDK/version"
  echo "logic, so it refuses to proceed rather than weaken or bypass that gate (batch_exact_jdk_gate_unreachable)."
  exit 1
fi
if ! command -v cygpath >/dev/null 2>&1; then
  echo "[BATCH_FAIL] cygpath not available -- cannot reliably hand this repo path to cmd.exe without"
  echo "risking a mis-invocation that silently skips verify-standalone.bat (batch_exact_jdk_gate_unreachable)."
  exit 1
fi
...
# 출력에 실제로 있는지까지 함께 확인해야 exact-JDK 게이트를 통과한 것으로 인정한다.
if [ "$STANDALONE_EXIT" -ne 0 ] || ! grep -q "\[STANDALONE_VERIFICATION_PASS\]" "$STANDALONE_LOG"; then
  echo "[BATCH_FAIL] verify-standalone.bat did not pass -- exact-JDK gate/compile/regression must pass"
  echo "before batch conversion begins (batch_exact_jdk_gate_not_passed). See its output above."
  rm -f "$STANDALONE_LOG"
  exit 1
fi
rm -f "$STANDALONE_LOG"
