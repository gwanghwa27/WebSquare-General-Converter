#!/bin/sh
# legacy XPlatformProjectConverter 호출을 제거하고 비활성화된 entrypoint로 전환됨.
# 범용 배치 CLI를 위한 Reviewer-approved 기본 TargetRuntimeProfile이 없어 default를
# 발명하지 않는다 -- TargetWebSquarePipeline.convert()를 직접 호출해야 한다.
set -u
